...
class CreateOptimizationRule(BaseModel):
    ...
    """ # noqa: E501
    state: StrictStr = Field(description="The state of the optimization rule.")
    rule_name: Optional[StrictStr] = Field(default=None, description="The name of the optimization rule.", alias="ruleName")
    rule_conditions: Annotated[List[RuleCondition], Field(min_length=1, max_length=1)] = Field(description="A list of rule conditions that define the advertiser's intent for the outcome of the rule. The rule uses 'AND' logic to combine every condition in this list, and will validate the combination when the rule is created or updated.", alias="ruleConditions")
    __properties: ClassVar[List[str]] = ["state", "ruleName", "ruleConditions"]
